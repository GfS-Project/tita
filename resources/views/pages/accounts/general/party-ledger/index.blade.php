        ...
        <div class="container-fluid">
            <div class="table-header justify-content-end border-0 p-0">
                <div class="button-group nav" role="tablist">
                    <a href="{{ route('party-ledger.index', ['type' => 'buyer']) }}" class="add-report-btn {{ request('type') == 'buyer' ? 'active' : '' }}">Buyer</a>
                    <a href="{{ route('party-ledger.index', ['type' => 'supplier']) }}" class="add-report-btn {{ request('type') == 'supplier' ? 'active' : '' }}">Supplier</a>
                </div>
            </div>
            <div class="tab-content order-summary-tab">
                <div class="tab-pane fade active show" id="payment-status" role="tabpanel">
                    <div class="table-header">
                        <h4>{{ ucfirst(request('type')) }} {{ __('Ledger Details') }}</h4>
                    </div>

                    <div class="total-count-area">
                        <div class="count-item light-blue">
                            <h5>{{ currency_format($total_bill) }}</span></h5>
                            <p>Total Bill</p>
                        </div>
                        <div class="count-item light-orange">
                            <h5>{{ currency_format($pay_amount) }}</span></h5>
                            <p>Total Payment</p>
                        </div>
                        <div class="count-item light-green">
                            <h5>{{ currency_format($advance_amount) }}</span></h5>
                            <p>Advance Amount</p>
                        </div>
                        <div class="count-item light-red">
                            <h5>{{ currency_format($due_amount) }}</span></h5>
                            <p>Due Amount</p>
                        </div>
                    </div>
                    <div class="table-top-form daily-transaction-between-wrapper">
                        <form action="{{ route('party-ledger.filter', ['type' => request('type')]) }}" method="POST" class="filter-form" table="#party-ledger-data">
                            @csrf
                            <div class="grid-5">
                        ...
                        <div class="table-top-btn-group">
                            <ul>
                                ...
                                <li>
                                    <a id="download-xlsx" data-filename="{{ 'Party-Ledger' . formatted_date(now(), "d/m/Y H:i:s") }}" data-table-headings="SL., Party Name, Party Type, Total Bill, Pay Amount, Advance Amount, Due Amount, Balance">
                    ...
                    <div class="responsive-table">
                        <div class="table-header justify-content-center border-0 d-block text-center print-inner-page">
                            ...
                            <h4 class="mt-2">{{ ucfirst(request('type')) }} {{ __('Ledger Details') }}</h4>
                        </div>
                        <table class="table" id="erp-table">
                            ...
                            <tbody id="party-ledger-data">
                                @include('pages.accounts.general.party-ledger.datas')
                            </tbody>
                        </table>
                    </div>
                    <nav>
                        <ul class="pagination">
                            <li class="page-item">{{ $parties->links() }}</li>
                        </ul>
                    </nav>
                </div>
            </div>
