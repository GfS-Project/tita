    ...
    <div class="erp-table-section">
        <div class="container-fluid">
            <div class="table-header">
                <h4>{{__('Monthly Transaction')}}</h4>
            </div>
            <div class="total-count-area mb-5">
                <div class="count-item light-blue">
                    <h5 id="transaction_total_amount">{{ $total_amount ?? 0 }}</h5>
                    <p>{{__('Total Amount')}}</p>
                </div>
                <div class="count-item light-green">
                    <h5 id="transaction_credit_amount">{{ $credit_amount ?? 0 }}</h5>
                    <p>{{__('Credit Amount')}}</p>
                </div>
                <div class="count-item light-orange">
                    <h5 id="transaction_debit_amount">{{ $debit_amount ?? 0 }}</h5>
                    <p>{{__('Debit Amount')}}</p>
                </div>
            </div>
            <div class="tab-content order-summary-tab">
                <div class="tab-pane fade show active" id="costing-list">
                    <div class="table-header">
                        <h4>{{__('Month of')}} {{ now()->format('F') }}</h4>
                    </div>
                    <div class="table-top-form daily-transaction-between-wrapper">
                        <form action="{{ route('transaction.filter') }}" method="POST" id="transaction_filter" class="transaction-filter-form" table="#transaction-data">
                            @csrf
                            <div class="grid-4">
                                <div class="input-wrapper">
                                    <select class="form-control m-0" name="year">
                                ...
                                <div class="input-wrapper">
                                    <select class="form-control m-0" name="month">
                                        @for ($i = 1; $i <= 12; $i++)
                                            <option value="{{ $i }}" {{ now()->format('n') == $i ? 'selected' : '' }}>
                        ...
                        <div class="table-top-btn-group">
                            <ul>
                                <li>
                                    <a href="{{ route('monthly-transactions.pdf') }}">
                                ...
                                <li>
                                    <a id="download-csv" data-filename="{{ 'Monthly-Transactions' . formatted_date(now(), "d/m/Y H:i:s") }}" data-table-headings="Date, Total Transaction, Total Amount, Type, Remarks">
                                        <svg class="w-17 h-16" viewBox="0 0 17 16" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                ...
                                <li>
                                    <a id="download-xlsx" data-filename="{{ 'Monthly-Transactions' . formatted_date(now(), "d/m/Y H:i:s") }}" data-table-headings="Date, Total Transaction, Total Amount, Type, Remarks">
                    ...
                    <div class="responsive-table table-sm">
                        <div class="table-header justify-content-center border-0 d-block text-center print-inner-page">
                            ...
                            <h4 class="mt-2">{{ __('Monthly Transaction List') }}</h4>
                        </div>
                        <table class="table" id="erp-table">
                            ...
                            @include('pages.accounts.general.transactions.datas')
                            </tbody>
                        </table>
                    </div>
                    <nav>
