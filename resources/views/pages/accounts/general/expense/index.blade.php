        ...
        <div class="container-fluid">
            <div class="table-header justify-content-end border-0 p-0">
                <div class="button-group nav">
                    @can('expense-read')
                    <a href="#income-list" class="add-report-btn {{ count($errors) ? '' : 'active' }}" data-bs-toggle="tab">{{__('Expense List')}}</a>
                    @endcan
                    @can('expense-create')
                    <a href="#income-add" class="add-report-btn {{ count($errors) ? 'active' : '' }}" data-bs-toggle="tab">{{__('Add Expense Category')}}</a>
                    @endcan
                </div>
            </div>
            <div class="tab-content order-summary-tab">
                                    ...
                                    <a href="{{ route('exports.index', ['model' => 'Expense', 'extension' => 'csv']) }}">
                                        <svg class="w-17 h-16" viewBox="0 0 17 16" fill="none"
                                            ...
                                            <path d="M6.0551 9.61807C6.1231 9.48573 6.2081 9.38107 6.3101 9.3034C6.4121 9.22607 6.5311 9.1874 6.6671 9.1874C6.91277 9.1874 7.11277 9.29873 7.26777 9.52173L7.65877 9.04573C7.5491 8.88707 7.40744 8.7654 7.23377 8.68007C7.0601 8.59507 6.85777 8.55273 6.62744 8.55273C6.40444 8.55273 6.20144 8.60473 6.0181 8.7084C5.83477 8.8124 5.6791 8.95773 5.55044 9.14473C5.4221 9.33207 5.32277 9.55473 5.2531 9.8134C5.1831 10.0724 5.14844 10.3547 5.14844 10.6607C5.14844 10.9667 5.18344 11.2481 5.2531 11.5051C5.32277 11.7621 5.4211 11.9837 5.54777 12.1707C5.67444 12.3581 5.8291 12.5044 6.01244 12.6101C6.19577 12.7157 6.40077 12.7687 6.62744 12.7687C6.87677 12.7687 7.08344 12.7197 7.24777 12.6214C7.4121 12.5231 7.5471 12.3947 7.65277 12.2361L7.26177 11.7771C7.19744 11.8754 7.1171 11.9584 7.0211 12.0264C6.9251 12.0944 6.81044 12.1284 6.67844 12.1284C6.53877 12.1284 6.41677 12.0897 6.31277 12.0121C6.20877 11.9347 6.1231 11.8301 6.05477 11.6977C5.98644 11.5654 5.93577 11.4097 5.90177 11.2301C5.86777 11.0507 5.85077 10.8611 5.85077 10.6607C5.85077 10.4567 5.86777 10.2651 5.90177 10.0854C5.9361 9.90607 5.9871 9.7504 6.0551 9.61807Z"
                                                fill="#00A551"/>
                                            <path d="M11.6637 11.8735H11.6467L10.9781 8.65479H10.2188L11.1991 12.6668H12.0604L13.0577 8.65479H12.3324L11.6637 11.8735Z"
                                                fill="#00A551"/>
                                        </svg>
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('exports.index', ['model' => 'Expense', 'extension' => 'xlsx']) }}">
                    ...
                    <div class="responsive-table">
                        ...
                        <table class="table" id="erp-table">
                            ...
                            @foreach($expenses as $expense)
                                <tr>
                                    <td>{{ $loop->iteration }} <i class="{{ request('id') == $expense->id ? 'fas fa-bell text-red' : '' }}"></i></td>
                                    <td>{{ $expense->category_name }}</td>
                                    <td>
                                        {{ \Illuminate\Support\Str::limit(strip_tags($expense->expense_description),50) }}
                                    </td>
                                    <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input status-item" type="checkbox"  name="status_{{$expense->id}}" id="status_{{$expense->id}}" value="{{$expense->status}}"  {{$expense->status==1?'checked':''}} data-id ="{{$expense->id}}" data-status="Expense" >
                                        </div>
                                    </td>
                                    <td class="print-d-none">
                                        <div class="dropdown table-action">
                                            ...
                                            <ul class="dropdown-menu">
                                                    @can('expense-update')
                                                    <li><a  href="{{ route('expense.edit',$expense->id) }}"><i class="fal fa-pencil-alt"></i>{{__('Edit')}} </a></li>
                                                    @endcan
                                                    @can('expense-delete')
                                                    <li>
                                                        <a href="{{ route('expense.delete', $expense->id) }}" class="confirm-action" data-method="DELETE">
                ...
                <div class="tab-pane fade {{ count($errors) ? 'show active' : '' }}" id="income-add">
                    <div class="table-header">
                        <h4>{{__('Add New Expense Category')}}</h4>
                    </div>
                    <div class="order-form-section col-xxl-6 col-xl-6 col-lg-6">
                        {{-- form start --}}
                        <form action="{{ route('expense.store') }}" method="post" enctype="multipart/form-data" class="ajaxform_instant_reload">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-lg-12 mt-2">
                                    ...
                                    <textarea name="expense_description" class="form-control" placeholder="Description">{{ old('expense_description') }}</textarea>
                                </div>
                                <div class="col-lg-12">
